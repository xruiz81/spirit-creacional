...
public class SubTareaHilo extends Thread {
    ...
    public void run(){
    	...
    	try {
        	...
        	subTareadetalle = ((ArrayList<SubTareaDetalle>)subTarea.getDetalle()).get(subTarea.getDetalle().size()-1);
        	int contador = 0;
    		while (isContar()){
    			...
    			if ( isContar() ){    				
    				...
    				getProyecto().setSegundosTotales( getProyecto().getSegundosTotales()+1 );
    			...
    			//grabo el tiempo automáticamente cada 5 minutos
    			/*if(contador == SECONDS){
    				AutoSave guardadoAutomatico = new AutoSave(getProyecto(),getSubTarea());
