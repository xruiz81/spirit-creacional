...
@Stateless
public class ComprobanteIngresoPosAsientoAutomaticoHandler implements ComprobanteIngresoPosAsientoAutomaticoHandlerLocal {
	...
	private String facturaPreimpreso = "";
	...
	public AsientoIf generarComprobanteIngresoAsientoAutomaticoHandler(EventoContableIf eventoContable, Map parameterMap, boolean procesarAsiento, Map<String,Object> parametrosEmpresa, Long etapa) throws GenericBusinessException {
		...
		this.facturaPreimpreso = (parameterMap.get("FACTURA_PREIMPRESO") != null)?(String)parameterMap.get("FACTURA_PREIMPRESO"):"S/N";
	...
	public AsientoIf generarFaltantesMultasSobrantesCajaAsientoAutomaticoHandler(EventoContableIf eventoContable, Map parameterMap, boolean procesarAsiento, Map<String,Object> parametrosEmpresa, Long etapa) throws GenericBusinessException {
		...
		this.referenceBean = parameterMap.get("BEAN");
		//this.carteraDetalle = parameterMap.get("CARTERA_DETALLE");
		this.carteraDetalle = null;
		this.facturaPreimpreso = null;
		//this.facturaPreimpreso = (parameterMap.get("FACTURA_PREIMPRESO") != null)?(String)parameterMap.get("FACTURA_PREIMPRESO"):"S/N";
		this.parameterMap = parameterMap;
		this.procesarAsiento = procesarAsiento;
		return generarAsientoFaltantesMultasSobrantesContable();
	}
	
	
	private AsientoIf registrarAsientoMultasFaltantes(TipoDocumentoIf tipoDocumento,String cajeroObservacion) throws GenericBusinessException {
		AsientoData data = new AsientoData();
		try {		
		
			java.util.Date fechaHoy = new java.util.Date();
			java.sql.Date fechaAsiento =new java.sql.Date(fechaHoy.getYear(), fechaHoy.getMonth(), fechaHoy.getDate());
			...
			if (validateAsientoAutomatico(idEmpresa, fechaAsiento)) {
				...
				String observacion=cajeroObservacion;
				if(observacion!=null) 
					data.setObservacion(observacion);
				else
					data.setObservacion(tipoDocumento.getCodigo()+ ": "+tipoDocumento.getNombre());
	...
	public AsientoIf generarAsientoFaltantesMultasSobrantesContable() throws GenericBusinessException {
		...
		try {
			...
			List<AsientoDetalleIf> asientoDetalleColeccionFaltantesMultas = new ArrayList<AsientoDetalleIf>();
			asientoDetalleColeccionFaltantesMultas.clear();
			String observacion="";
			
			observacion = ((String) this.parameterMap.get("OBSERVACION")).toString();
			...
			while (plantillasIterator.hasNext()) {
				...
				System.out.println("plantilla>>"+plantilla.getNemonico());
				
								
				AsientoDetalleIf asientoDetalle = new AsientoDetalleData(); 
				asientoDetalle.setCuentaId(cuenta.getId());
				String glosa ="Documento: "+documento.getCodigo()+" cuenta: "+cuenta;
				asientoDetalle.setGlosa(glosa );				
				...
				if (valor > 0.0){
					asientoDetalleColeccionFaltantesMultas.add(asientoDetalle);
				}
			}
			
			if (procesarAsiento && (asientoDetalleColeccionFaltantesMultas.size() > 0)) {
				AsientoIf asiento = registrarAsientoMultasFaltantes(tipoDocumento,observacion);
				if (asiento != null) {
					if (eventoContable.getAgruparDetalles().equals("S"))
						asientoDetalleColeccionFaltantesMultas = asientoLocal.agruparDetalles(asientoDetalleColeccionFaltantesMultas);
					String numeroAsiento = asientoLocal.procesarAsiento(asiento, asientoDetalleColeccionFaltantesMultas, true);
		...
		try {
			...
			while (plantillasIterator.hasNext()) {
				...
				if (documento.getCodigo().equals("REIC") || documento.getCodigo().equals("RERC")) {
					...
					datosFactura = this.facturaPreimpreso;
				} else if(((CarteraIf) referenceBean).getCodigo().length() >= 20) {
					String codigoTipoDocumento = "I: ";
					if (!tipoDocumento.getCodigo().equals("CIN"))
						codigoTipoDocumento = tipoDocumento.getCodigo() + ": ";
					glosa = codigoTipoDocumento + ((CarteraIf) referenceBean).getCodigo().substring(12,25) + " " + glosa;
				} else {
					String codigoTipoDocumento = "I: ";
					if (!tipoDocumento.getCodigo().equals("CIN"))
						codigoTipoDocumento = tipoDocumento.getCodigo() + ": ";
			...
			if (procesarAsiento && (asientoDetalleColeccion.size() > 0)) {
				...
				if (asiento != null) {
					if (eventoContable.getAgruparDetalles().equals("S")) {
						...
						asientoDetalleColeccion = asientoLocal.desagruparDetalles(asientoDetalleColeccion);
					}
		...
		try {		
			...
			if (validateAsientoAutomatico(idEmpresa, fechaAsiento)) {
				...
				if(((CarteraIf) referenceBean).getCodigo().length() >= 20){
					data.setObservacion(codigoTipoDocumento + ((CarteraIf) referenceBean).getCodigo().substring(12,25) + " " + cliente.getRazonSocial());
				} else {
	...
	public void setAsientoDetalleColeccion(List<AsientoDetalleIf> asientoDetalleColeccion) {
		ComprobanteIngresoPosAsientoAutomaticoHandler.asientoDetalleColeccion = asientoDetalleColeccion;
	}	
}
