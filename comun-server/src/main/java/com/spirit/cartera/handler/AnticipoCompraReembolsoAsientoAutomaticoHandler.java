...
@Stateless
public class AnticipoCompraReembolsoAsientoAutomaticoHandler implements AnticipoCompraReembolsoAsientoAutomaticoHandlerLocal{
	...
	public AsientoIf generarAsientoContable() throws GenericBusinessException {
		...
		try {
			...
			while (plantillasIterator.hasNext()) {
				...
				asientoDetalle.setGlosa("ANT. COMP. REEMB. # " + ((CarteraIf) referenceBean).getCodigo() + " " + ((CarteraIf) referenceBean).getComentario());
			...
			if (procesarAsiento && (asientoDetalleColeccion.size() > 0)) {
				...
				if (asiento != null) {
					...
					if ( asientosRetornar!=null && asientosRetornar.iterator().hasNext() ){
						...
						throw new GenericBusinessException("Error en Handler al generar asiento para Anticipo Compra Reembolso");
				}
			}
		} catch (GenericBusinessException e) {
			e.printStackTrace();
	...
	public AsientoIf registrarAsiento() {
		...
		try {			
			...
			if (validateAsientoAutomatico(empresaId, fechaAsiento)) {
				...
				data.setObservacion("ANT. COMP. REEMB. # " + ((CarteraIf) referenceBean).getCodigo() + " " + parameterMap.get("OBSERVACION").toString());
	...
	public static void setAsientoMap(Map asientoMap) {
		AnticipoCompraReembolsoAsientoAutomaticoHandler.asientoMap = asientoMap;
	}
	...
	public void setAsientoDetalleColeccion(List<AsientoDetalleIf> asientoDetalleColeccion) {
		AnticipoCompraReembolsoAsientoAutomaticoHandler.asientoDetalleColeccion = asientoDetalleColeccion;
	}	
}
