...
@Stateless
public class TransferenciaDocumentoAsientoAutomaticoHandler implements TransferenciaDocumentoAsientoAutomaticoHandlerLocal {
	...
	private Object transferencia = null;
	private ClienteIf cliente = null;

	public AsientoIf generarTransferenciaDocumentoAsientoAutomaticoHandler(EventoContableIf eventoContable, Map parameterMap, boolean procesarAsiento, Map<String,Object> parametrosEmpresa) throws GenericBusinessException {
		...
		this.transferencia = parameterMap.get("TRANSFERENCIA");
		...
		try {
			...
			while (plantillasIterator.hasNext()) {
				...
				if (cuenta.getImputable().equals("N")) {
					...
					} else if (plantilla.getNemonico().startsWith("CTAXPAG")) {
						parameterMap.put("entidadId", operadorNegocio.getId());
				...
				String observacion = "TRANSF. " + tipoDocumento.getCodigo() + ": ";
				...
				if(((CarteraIf) referenceBean).getCodigo().length() >= 20) {
					...
					if (((CarteraIf) referenceBean).getPreimpreso() != null && !((CarteraIf) referenceBean).getPreimpreso().equals(""))
						observacion += ((CarteraIf) referenceBean).getPreimpreso();
					else
						observacion += codigoCartera;
				} else {
					observacion += ((((CarteraIf) referenceBean).getPreimpreso() != null && !((CarteraIf) referenceBean).getPreimpreso().equals(""))?((CarteraIf) referenceBean).getPreimpreso():((CarteraIf) referenceBean).getCodigo());
				}

				asientoDetalle.setGlosa(observacion + " " + ((CarteraIf) referenceBean).getComentario());
			...
			if (procesarAsiento && (asientoDetalleColeccion.size() > 0)) {
				...
				if (asiento != null) {
					...
					if ( asientosRetornar!=null && asientosRetornar.iterator().hasNext() ){
						...
						throw new GenericBusinessException("Error en Handler al generar asiento para transferencia de documento");
				}
			}
		} catch (GenericBusinessException e) {
			e.printStackTrace();
	...
	private AsientoIf registrarAsiento(TipoDocumentoIf tipoDocumento) throws GenericBusinessException {
		...
		try {		
			java.sql.Date fechaAsiento = utilitariosLocal.fromTimestampToSqlDate(((CarteraIf) transferencia).getFechaEmision());
			if (validateAsientoAutomatico(idEmpresa, fechaAsiento)) {
				...
				data.setObservacion(observacion + " " + ((CarteraIf) referenceBean).getComentario());
				...
				data.setTransaccionId(((CarteraIf) transferencia).getId());
				data.setElaboradoPorId(((CarteraIf) transferencia).getUsuarioId());
	...
	public void setAsientoDetalleColeccion(List<AsientoDetalleIf> asientoDetalleColeccion) {
		TransferenciaDocumentoAsientoAutomaticoHandler.asientoDetalleColeccion = asientoDetalleColeccion;
	}	
}
