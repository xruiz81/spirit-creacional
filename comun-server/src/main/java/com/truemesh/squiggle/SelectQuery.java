...
public class SelectQuery implements Outputable, ValueSet {
	...
	public void addObject(Table table) {
		addToSelection(table.getAliasObject());
	}
	...
	public void addParameter(Table table, String columnName) {
		this.criteria.add(new ParameterCriteria(table, columnName));
	}
	...
	public void addJoin(Table srcTable, String srcColumnname, Table destTable,
		...
		addCriteria(new MatchCriteria(srcTable.getColumn(srcColumnname),
				MatchCriteria.EQUALS, destTable.getColumn(destColumnname)));
	}

	/**
	 * Syntax sugar for addCriteria(JoinCriteria)
	 */
	public void addJoin(Table srcTable, String srcColumnName, String operator,
			Table destTable, String destColumnName) {
		addCriteria(new MatchCriteria(srcTable.getColumn(srcColumnName),
				operator, destTable.getColumn(destColumnName)));
	}
	...
	private void appendIndentedList(Output out,
			Collection<? extends Outputable> things, String seperator) {
		out.indent();
	 ...
	 * to a StringBuffer.
	 */
	private void appendList(Output out,
			Collection<? extends Outputable> collection, String seperator) {
