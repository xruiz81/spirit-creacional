...
import com.spirit.cartera.gui.panel.JPCruceAnticipoCliente;
...
public class CruceAnticipoClienteModel extends JPCruceAnticipoCliente {
	...
	private int selectedAnticipo = -1;
	private List<CarteraIf> cuentasPorCobrarColeccion = new ArrayList<CarteraIf>();
	private List<CarteraIf> anticiposColeccion = new ArrayList<CarteraIf>();
	private CarteraIf cuentaPorCobrarForCruce;
	private CarteraIf anticipoClienteForCruce;
	...
	public CruceAnticipoClienteModel() {
		setSorterTable(getTblCuentasPorCobrar());
		setSorterTable(getTblAnticipos());
		getTblCuentasPorCobrar().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		getTblAnticipos().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		initListeners();
		iniciarComponentes();
		showSaveMode();
		setColumnsWidth();
	...
	private void setColumnsWidth() {
		...
		// tblCuentasPorPagar
		anchoColumna = getTblAnticipos().getColumnModel().getColumn(0);
		anchoColumna.setPreferredWidth(100);
		anchoColumna = getTblAnticipos().getColumnModel().getColumn(1);
		anchoColumna.setPreferredWidth(100);
		anchoColumna = getTblAnticipos().getColumnModel().getColumn(2);
		anchoColumna.setPreferredWidth(300);
		anchoColumna = getTblAnticipos().getColumnModel().getColumn(3);
		anchoColumna.setPreferredWidth(50);
		
		getTblCuentasPorCobrar().getTableHeader().setReorderingAllowed(false);
		getTblAnticipos().getTableHeader().setReorderingAllowed(false);
	...
	private void seleccionarAnticipo(int selectedRow) {
		setAnticipoSeleccionado(selectedRow);
		anticipoClienteForCruce = (CarteraIf) anticiposColeccion.get(getTblAnticipos().convertRowIndexToModel(getAnticipoSeleccionado()));
	}
	...
	private void cargarTablaAnticipos() {
		try {
			anticiposColeccion.clear();
			DefaultTableModel tableModel = (DefaultTableModel) getTblAnticipos().getModel();
			java.util.Date fecha = new java.util.Date();
			java.sql.Date fechaCorte = new java.sql.Date(fecha.getYear(),fecha.getMonth(),fecha.getDate());
			ArrayList anticiposList = (ArrayList) SessionServiceLocator.getCarteraSessionService().findAnticiposClientePorCruzar(Parametros.getIdEmpresa(), operadorNegocioOficina.getClienteId(), fechaCorte);
			Iterator it = anticiposList.iterator();
			
			while (it.hasNext()) {
				CarteraIf cartera = (CarteraIf) it.next();
				if (cartera.getSaldo().doubleValue() > 0D) {
					Vector<Object> fila = new Vector<Object>();
					fila.add(cartera.getCodigo());
					fila.add(cartera.getFechaEmision());
					fila.add(cartera.getComentario()!=null?cartera.getComentario():"");
					fila.add(formatoDecimal.format(Utilitarios.redondeoValor(cartera.getSaldo().doubleValue())));
					tableModel.addRow(fila);
					anticiposColeccion.add(cartera);
				}
			}
		} catch (GenericBusinessException e) {
			e.printStackTrace();
	...
	public void save() {
		if (validateFields()) {
			...
			if (cuentasPorCobrarSeleccionadas.size() > 0) {
				if (cruzarCuentasPorCobrar(cuentasPorCobrarSeleccionadas, valoresAplica)) {
	...
	private boolean cruzarCuentasPorCobrar(List<CarteraIf> cuentasPorCobrarSeleccionadas, Map valoresAplica) {
		try {
			int selectedRow = getTblAnticipos().getSelectedRow();
			seleccionarAnticipo(selectedRow);
			CarteraIf anticipo = anticipoClienteForCruce;
			if (anticipo.getSaldo().doubleValue() >= valorAfecta) {
				Map parameterMap = new HashMap();
				parameterMap.put("idEmpresa", Parametros.getIdEmpresa());
				parameterMap.put("idOficina", Parametros.getIdOficina());
				...
				double saldoCuentaPorPagar = anticipo.getSaldo().doubleValue();
				...
				SessionServiceLocator.getCarteraSessionService().procesarCruceAnticipoCliente(cuentasPorCobrarSeleccionadas, anticipo, valoresAplica, parameterMap);
			} else {
				SpiritAlert.createAlert("No se puede realizar el cruce!\nEl valor a cruzar supera al saldo del anticipo!", SpiritAlert.WARNING);
				return false;
			}
		} catch (GenericBusinessException e) {
			e.printStackTrace();
	...
	public boolean validateFields() {
		...
		if (getTblAnticipos().getSelectedRow() < 0) {
			SpiritAlert.createAlert("Debe seleccionar el anticipo a cruzar!", SpiritAlert.WARNING);
			return false;
		}
	...
	private int getAnticipoSeleccionado() {
		return this.selectedAnticipo;
	}
	
	private void setAnticipoSeleccionado(int selectedAnticipo) {
