...
import com.spirit.nomina.gui.panel.JPRubrosEventualesContrato;
...
public class RubrosEventualesContratoModel extends JPRubrosEventualesContrato {
	...
	public RubrosEventualesContratoModel() {
		iniciarComponentes();
		initListeners();
		anchoComlumnas();
		initKeyListeners();
	...
	private void iniciarComponentes(){
		...
		getCmbFechaCobro().setFormat(Utilitarios.setFechaMesAnioUppercase());
	...
	private void cargarTipoRol(){
		try {
			Collection<TipoRolIf> tipoRoles = SessionServiceLocator.getTipoRolSessionService().findTipoRolByRubroEventual("S");
	...
	public void agregarRubroEventual() {
		try{
			...
			if (validateFieldsRubroEventual()) {
				...
				TipoRolIf tipoRolQuincenal = buscarTipoRol("QUINCENA");
				TipoRolIf tipoRolMensual = buscarTipoRol("MENSUAL");
				...
				if ( numeroPagosString!=null && !"".equals(numeroPagosString) ){
					...
					TipoRol tipoRol = TipoRol.obtenerTipoRol(tipoRolCobro);
					...
					if ( tipoRol == TipoRol.QUINCENAL ){
						for ( int i=1; i<numeroPagos+1; i++ ){
							
							if ( i%2 != 0 ){
								
								if ( getRbFinMesSi().isSelected() && numeroPagos>1 ){
									tipoRolPago = tipoRolMensual;
									fechaPago = calendarCombo.getTime();
								} else {
									tipoRolPago = tipoRolQuincenal;
								}
								
								crearRubroEventual(calendarCombo.getTime(), " "+i+"/"+numeroPagos,tipoRolQuincenal,fechaPago,tipoRolPago,identificacionMaxima);
							} else{
								crearRubroEventual(calendarCombo.getTime(), " "+i+"/"+numeroPagos,tipoRolMensual,fechaPago,tipoRolPago,identificacionMaxima);
								calendarCombo.add(Calendar.MONTH,1);
							}
						}
						...
						throw new GenericBusinessException("Tipo de Rol no considerado para división de pagos !!");
				} else {
			...
			if ( estadoRubroEventual.length == 1 )
				rubrosEventualesColleccion =  (ArrayList) SessionServiceLocator.getRubroEventualSessionService()
					.findRubroEventualByQueryByEstados(
						mapaRubrosEventuales,mapaFechas,null,true,TipoRubro.EVENTUAL.getLetra(),
	...
	private Object[] crearFilaTabla( RubroEventualIf rubroEventual ) throws GenericBusinessException{
		...
		Object[] fila = { 	
				nombreRubro,
				Utilitarios.getFechaMesAnioUppercase(rubroEventual.getFechaCobro()),
	...
	private void cargarComboRubroEventual(){
		try {
			...
			ArrayList<RubroIf> rubrosEventuales = (ArrayList<RubroIf>) SessionServiceLocator.getRubroSessionService()
				.findRubroByQueryByTiposRubro(mapa,
					TipoRubro.EVENTUAL.getLetra(),TipoRubro.ADELANTO.getLetra() );
			for ( RubroIf r : rubrosEventuales ){
				rubros.add(r);
				mapaRubroEventual.put(r.getId(), r);
			}
			
			ArrayList<RubroIf> rubrosComisiones = (ArrayList<RubroIf>) SessionServiceLocator.getRubroSessionService()
			.findRubroByQueryByTiposRubro(mapa,
				TipoRubro.COMISION.getLetra(),TipoRubro.ADELANTO.getLetra() );
			
			for ( RubroIf r : rubrosComisiones ){
				rubros.add(r);
				mapaRubroEventual.put(r.getId(), r);
			}
	...
	private void cargarComboTipoRol(){
		try {
			...
			getCmbTipoRolCobro().setSelectedItem(null);
			getCmbTipoRolCobro().repaint();
			refreshCombo(getCmbTipoRolPago(), tiposRol);
			getCmbTipoRolPago().setSelectedItem(null);
			getCmbTipoRolPago().repaint();
		} catch (GenericBusinessException e) {
			e.printStackTrace();
