	...
	public TableScrollPaneMapaPauta(String[] cabecerasFijas, String[] cabecerasVariables, Object[][] datosPrensa, int numeroFilas){
        ...
        TableCellRendererHorizontalRightAlignment tableCellRenderer = new TableCellRendererHorizontalRightAlignment();
   	 	for (int i = 0; i < footerColumns; i++) {
   	 		//seteo el ancho de las celdas del mapa de pauta
       	 	table.getMainTable().getColumnModel().getColumn(i).setPreferredWidth(40);
       	 	//seteo alineación a la derecha de las celdas del mapa de pauta
       	 	table.getMainTable().getColumnModel().getColumn(i).setCellRenderer(tableCellRenderer); 	
       	 	//seteo alineación a la derecha de los totales por dia de las celdas del mapa de pauta
       	 	table.getColumnFooterTable().getColumnModel().getColumn(i).setCellRenderer(tableCellRenderer); 	
        }
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            if (columnIndex == 0) {
                ...
                for (int i = headerColumns; i <= (HEADER.length-2); i++) {
                    try {
                        try {
                        	String valor = (String)ENTRIES[rowIndex][i];                    		
                        	if(valor.length() > 0){
                        		if(valor.contains("[")){
                        			for (int contador = 0 ; contador < valor.length() ; contador++){
                        				char caracter = valor.charAt(contador);
                            ...
                            //total += Integer.valueOf((String) ENTRIES[rowIndex][i]);
                        }
	...
	class TimeSheetTotalTableModel extends AbstractMultiTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            for (int i = 0; i < _model.getRowCount(); i++) {
                try {
                	String valor = (String)_model.getValueAt(i, columnIndex + headerColumns);            		
                    if(valor.length() > 0){
                		if(valor.contains("[")){
                			for (int contador = 0 ; contador < valor.length() ; contador++){
                				char caracter = valor.charAt(contador);
